...
export async function getPostByTrend(req, res) {
    try {
        ...
        const trends = await connection.query(`SELECT users.name AS "username",users."pictureUrl" AS "usersPhoto",posts.link AS "link", posts."likeQtd",  posts.description AS "description", "postImage".* FROM users JOIN posts ON  users.id = posts."userId" JOIN trends ON posts.id = trends."postId" JOIN "postImage" ON posts.id = "postImage"."postId" WHERE trends.trend = $1;`, [queryHashtag])
        if (trends.rowCount === 0) {
            res.status(401).send([])
            return
        }
